lasso_simulation <- function(X.train,Y.train,X.test,Y.test,beta){
  ...
  lambda = seq(0.01,10, length.out = 1000) # If want to provide lambda sequence by ourselves
  AIC_values = data.frame(lambda = NULL , AIC = NULL)
  
  for (l in lambda){
    aic.lasso <- glmnet(X.train, Y.train, alpha = 1, family = "binomial", lambda = l)
    
    new_row = data.frame(lambda = l, AIC =   aic_glmnet(aic.lasso))
